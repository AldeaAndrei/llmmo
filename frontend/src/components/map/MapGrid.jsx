...
function drawMap(canvas, selectedTile) {
  ...
  ctx.fillRect(
    HOME_TILE.x * CELL_SIZE,
    HOME_TILE.y * CELL_SIZE,
    CELL_SIZE,
    CELL_SIZE,
  )
}
...
function MapControls({ containerRef }) {
  ...
  const jumpToCity = useCallback(() => {
    ...
    const centerX = HOME_TILE.x * CELL_SIZE + CELL_SIZE / 2
    const centerY = HOME_TILE.y * CELL_SIZE + CELL_SIZE / 2
  ...
  return (
    <div className="absolute right-3 top-3 z-10">
      <Button type="button" variant="outline" size="sm" onClick={jumpToCity}>
        Jump to city
      </Button>
    </div>
  )
